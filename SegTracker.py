...
from tool.transfer_tools import draw_points
...
class SegTracker():
    ...
    def seg(self, frame):
        ...
        self.origin_merged_mask = np.zeros(
            anns[0]['segmentation'].shape, dtype=np.uint8)
        idx = 1
        for ann in anns:
        ...
        for id in obj_ids:
            if np.sum(self.origin_merged_mask == id) < self.min_area or self.object_idx > self.max_obj_num:
                ...
                self.origin_merged_mask[self.origin_merged_mask ==
                                        id] = self.object_idx
                self.object_idx += 1
    ...
    def add_reference(self, frame, mask, frame_step=0):
        ...
        self.tracker.add_reference_frame(
            frame, mask, self.curr_idx, frame_step)
    ...
    def seg_acc_bbox(self, origin_frame: np.ndarray, bbox: np.ndarray,):
        ...
        # draw bbox
        masked_frame = cv2.rectangle(
            masked_frame, bbox[0], bbox[1], (0, 0, 255))
    ...
    def seg_acc_click(self, origin_frame: np.ndarray, coords: np.ndarray, modes: np.ndarray, multimask=True):
        ...
        '''
        # get interactive_mask
        interactive_mask = self.sam.segment_with_click(
            origin_frame, coords, modes, multimask)
    ...
    def add_mask(self, interactive_mask: np.ndarray):
        ...
        if self.origin_merged_mask is None:
            self.origin_merged_mask = np.zeros(
                interactive_mask.shape, dtype=np.uint8)
